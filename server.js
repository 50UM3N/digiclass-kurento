        ...
        switch (message.event) {
            case "joinRoom":
                ...
                socket.on("disconnect", (e) => {
                    ...
                    if (io.sockets.adapter.rooms[message.roomName])
                        delete io.sockets.adapter.rooms[message.roomName]
                            .participants[socket.id];
                    socket.to(message.roomName).emit("dis", socket.id);
                });
                break;
...
function joinRoom(socket, username, roomname, callback) {
    getRoom(socket, roomname, (err, myRoom) => {
        ...
        myRoom.pipeline.create("WebRtcEndpoint", (err, outgoingMedia) => {
            ...
            socket.to(roomname).broadcast.emit("message", {
